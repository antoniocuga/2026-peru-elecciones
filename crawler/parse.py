...
def parse_candidate_names():
    ...
    df.to_csv(os.path.dirname(os.path.abspath(__file__))  + '/candidates_names.csv', index=False)


def parse_congreso():
    flag_as_old_everything_but_last('col_congreso','slug')
    spider = EG2021Spider("xx")
    ...
    fpath = os.path.dirname(os.path.abspath(__file__))  + "/congreso_sucio.csv"
    print(len(dfs),fpath)
...
def parse_summary():
    ...
    flag_as_old_everything_but_last()
    ...
    print(fpath, len(seen_ubigeos), flush=True)
    final.to_csv(fpath, index=False)
