...
congreso_total <- 
    congreso_total %>% 
        ...
        congreso_total %>% 
            group_by(partido) %>% 
            summarise(total_votos_congreso = sum(unique(total_votos_partido))) %>% 
            inner_join(select(prop_nacionales %>% ungroup(), total_partido, partido), by = "partido") %>% 
            mutate(voto_fantasma = total_partido - total_votos_congreso) %>% 
            select(partido, voto_fantasma),
        by = "partido"
    )
